...
def create_or_update_lead_from_csv_row(data_dict):
    ...
    Args:
        data_dict (dict): Dictionary containing lead data from CSV row.
                         ...
                         - first_name (required)
                         - last_name (required)
                         - pan_number (required, format: AAAAA9999A)
                         - pin_code (required, 6 digits)
                         - email (optional)
                         - date_of_birth or dob (optional, format: YYYY-MM-DD)
                         - gender (optional: Male/Female/Other)
                         - city, state (optional)
                         - profession (optional: Salaried/Self Employed/Student)
    ...
    if not pan_number:
        raise ValidationError('pan_number is required')
    
    pin_code = data.get('pin_code', data.get('pincode', '')).strip()
    if not pin_code:
        raise ValidationError('pin_code is required')
    if len(pin_code) != 6:
        pin_code = pin_code.zfill(6)  # Pad with zeros if needed
    
    # Prepare lead data (no lender association needed)
    lead_data = {
    ...
    if profession:
        ...
        lead_data['profession'] = prof_map.get(profession.lower(), profession)
    ...
    try:
        lead = Lead.objects.get(phone_number=phone_number)
